defmodule Mix.Tasks.Stac.ImportItems do
  ...
  defp import_items_dir(items_dir, opts) do
    ...
    unless check_collection_exists(base_url, collection_id) do
      if opts[:dry_run] do
        Mix.shell().info("[DRY RUN] Would create collection: #{collection_id}")
      else
        create_collection(base_url, collection_id, opts[:catalog_id])
      end
    end
    ...
    if opts[:dry_run] do
      ...
      count = import_items_via_api(base_url, items, collection_id)
  ...
  defp check_collection_exists(base_url, collection_id) do
    url = "#{base_url}/api/stac/v1/collections/#{collection_id}"
  ...
  defp create_collection(base_url, collection_id, catalog_id) do
    payload = %{
      id: collection_id,
      title: collection_id,
    ...
    url = "#{base_url}/api/stac/v1/collections"
